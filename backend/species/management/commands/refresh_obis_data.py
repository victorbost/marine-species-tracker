...
class Command(BaseCommand):
    help = "Triggers the refresh of OBIS data in a separate thread. Supports full or date-range refresh."
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            default=getattr(settings, 'OBIS_DEFAULT_GEOMETRY', "POLYGON((-180 -90, 180 -90, 180 90, -180 90, -180 -90))")
        )
        parser.add_argument(
            '--taxonid',
            type=str,
        ...
        parser.add_argument(
            ...
            help='Refresh mode: "full" (all data) or "incremental" (using date range, typically last month).'
        )
        parser.add_argument(
            '--start-date',
            type=str,
            help='Start date in YYYY-MM-DD format for filtering records. Used with --mode incremental. '
                 'Defaults to 1 month ago if --mode incremental and not specified.'
        )
        parser.add_argument(
            '--end-date',
            type=str,
            help='End date in YYYY-MM-DD format for filtering records. Used with --mode incremental. '
                 'Defaults to today if --mode incremental and --start-date specified but not this.'
        )
    ...
    def handle(self, *args, **options):
        ...
        if mode == 'incremental':
            ...
            self.stdout.write(f"Running in INCREMENTAL mode, fetching data with eventDate from {final_start_date} to {final_end_date}")
        else: # mode == 'full'
            self.stdout.write("Running in FULL REFRESH mode (no date filters applied).")
        ...
        etl_thread = threading.Thread(
            ...
            args=(geometry_wkt, taxonid, pages, final_start_date, final_end_date),
        )
        etl_thread.start()
        etl_thread.join()
