...
export default function MapComponent() {
  ...
  useEffect(() => {
    const fetchObservations = async () => {
      try {
        const response = await fetch(`${API_URL}/api/v1/map/observations/?lat=0&lng=0&radius=10000`, {
          credentials: 'include',
  ...
  return (
    <MapContainer center={defaultPosition} zoom={2} scrollWheelZoom={true} style={{ height: '500px', width: '100%' }}>
      <TileLayer
      ...
      {observations && observations.length > 0 && (
        observations.map((feature) => {
          ...
          const { id, species_name, common_name, observation_datetime, location_name, source } = feature.properties;

          return (
            <Marker key={id} position={[lat, lng]} icon={blueIcon}>
              <Popup>
                <div>
                  <strong>{species_name}</strong> ({source === 'obis' && common_name ? common_name : source})<br />
                  {location_name && `Location: ${location_name}`}<br />
                  {observation_datetime && `Observed: ${new Date(observation_datetime).toLocaleDateString()}`}
                </div>
              </Popup>
            </Marker>
          );
